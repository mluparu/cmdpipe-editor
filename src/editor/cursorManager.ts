...
import { OutputInsertionContext, InsertionMode } from '../types/extensionTypes';
...
export class CursorManager {
    ...
    /**
     * Create OutputInsertionContext for the current editor state
     */
    public createOutputInsertionContext(defaultInsertionMode: InsertionMode = InsertionMode.CURSOR): OutputInsertionContext | null {
        const activeEditor = vscode.window.activeTextEditor;
        
        if (!activeEditor) {
            return {
                editorUri: vscode.Uri.parse('untitled:untitled'),
                cursorPosition: new vscode.Position(0, 0),
                selectedRange: undefined,
                insertionMode: InsertionMode.OUTPUT_PANEL, // Fallback when no editor
                isReadonly: true,
                language: 'plaintext',
        ...
        // Determine appropriate insertion mode based on editor state
        let insertionMode = defaultInsertionMode;
        
        // If editor is readonly, force output panel
        if (isReadonly) {
            insertionMode = InsertionMode.OUTPUT_PANEL;
        }
        // If default is REPLACE_SELECTION but nothing is selected, use CURSOR
        ...
        return {
            ...
            cursorPosition,
            selectedRange: selection.isEmpty ? undefined : new vscode.Range(selection.start, selection.end),
            insertionMode,
            isReadonly,
            language,
    ...
    // /**
    //  * Get optimal insertion mode based on current editor state
    //  */
    // public getOptimalInsertionMode(): InsertionMode {
    //     const activeEditor = vscode.window.activeTextEditor;
    ...
    //     if (this.isReadonlyDocument(activeEditor.document)) {
    //         return InsertionMode.OUTPUT_PANEL;
    //     }
